with open('src/components/customer/CartTab.jsx', 'r') as f:
    content = f.read()

replacement = """export default function CartTab({ cart, walletBalance, setWalletBalance, orders, setOrders, transactions, setTransactions, setActiveTab, handleUpdateQty, handleRemoveItem, clearCart, totals }) {
      ...
      {cart.length === 0 ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-slate-50 dark:bg-slate-900/50 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 h-fit space-y-6">
            ...
            <button onClick={handleCheckout} className="w-full py-3 bg-primary text-white rounded-xl font-bold uppercase tracking-wide hover:bg-primary/90 transition-colors">
              Proceed to Checkout
            </button>
          </div>
        </div>
...
import re
content = re.sub(r'export default function CartTab\(\{.*?\}\) \{.*?\n\}', replacement, content, flags=re.DOTALL)
# wait, there's another block. I will just rewrite the whole file.

with open('src/components/customer/CartTab.jsx', 'w') as f:
