...
import { ShieldCheck, CreditCard, CheckCircle2, Clock, Info, AlertTriangle, ArrowRight, ArrowLeft, Lock, Landmark, QrCode, X } from 'lucide-react';
import { useWholesale, calculateOnboardingFee } from '../../../context/WholesaleContext';

export default function Step11_OnboardingFee({ onNext, onPrev }) {
  ...
  const [upiVpa, setUpiVpa] = useState('wholesale@upi');

  const isPaid = formData.onboardingPaymentCompleted;

  const handleOpenGateway = () => {
    if (isPaid) {
      addToast('Wholesale onboarding fee payment is already completed & verified!', 'info');
      onNext();
      return;
    }
    setIsModalOpen(true);
  ...
  const handleConfirmPayment = (e) => {
    ...
    setTimeout(() => {
      ...
      const payId = `PAY-WHL-${Math.floor(100000 + Math.random() * 900000)}`;
      updateFormData({
      ...
      addToast('One-Time Wholesaler Onboarding Fee Paid & Verified! Account Valid for 2 Years.', 'success');
      setTimeout(() => {
        onNext();
      }, 500);
    }, 1200);
  ...
  const handleProceedNext = () => {
    if (!formData.onboardingPaymentCompleted) {
      addToast('Please complete the One-Time Wholesaler Onboarding Fee payment first!', 'error');
      return;
    }
    onNext();
  };
      ...
      <div className="rounded-3xl border border-slate-200 dark:border-slate-800 bg-surface p-6 sm:p-10 shadow-xl">
        ...
        <div className="mt-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 dark:border-slate-800 pb-6">
          <div>
            <h2 className="text-2xl sm:text-3xl font-black text-slate-900 dark:text-white">
              One-Time Wholesaler Onboarding Fee
            </h2>
            <p className="mt-1 text-sm text-slate-600 dark:text-slate-400 font-medium">
              Mandatory registration & verification fee for all wholesale sellers joining SAATHAPP B2B network.
            </p>
          </div>
          <div className="text-right shrink-0 rounded-2xl bg-emerald-500/10 border border-emerald-500/20 p-3">
        ...
        <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="md:col-span-2 space-y-6">
            ...
            <div className="rounded-2xl border border-amber-500/30 bg-amber-500/10 p-4 text-xs space-y-2 text-amber-900 dark:text-amber-200">
              ...
              <p>
                • <strong>No Guarantee of Approval:</strong> Payment covers verification processing and does not guarantee automatic approval if compliance standards fail.
              </p>
            </div>
          </div>
          ...
          <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-page dark:bg-slate-950 p-5 flex flex-col justify-between space-y-4 shadow-sm">
            <div>
              ...
              <div className="space-y-2 text-xs">
                {[
                  ...
                  { id: 'NetBanking', label: 'NetBanking (HDFC/ICICI/SBI)', icon: Landmark },
                  ...
                  >
                    ...
                    {selectedMethod === method.id && <CheckCircle2 size={14} className="text-emerald-500" />}
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="pt-4 border-t border-slate-200 dark:border-slate-800 space-y-3">
              ...
              <button
                ...
                className={`transition-all duration-200 active:scale-[0.98] focus-visible:ring-2 focus-visible:ring-emerald-500/50 focus-visible:outline-none w-full flex items-center justify-center gap-2 rounded-2xl py-3.5 text-xs font-black transition shadow-lg ${
                  isPaid
        ...
        <div className="pt-8 flex items-center justify-between gap-4 border-t border-slate-200 dark:border-slate-800 mt-6">
          ...
          <button
            ...
            className={`transition-all duration-200 active:scale-[0.98] focus-visible:ring-2 focus-visible:ring-emerald-500/50 focus-visible:outline-none inline-flex items-center gap-2 rounded-2xl px-8 py-3.5 text-sm font-extrabold text-white transition ${
              isPaid
                ? 'bg-emerald-600 hover:bg-emerald-500 text-white shadow-lg hover:scale-105'
                : 'bg-slate-800 hover:bg-slate-700'
            }`}
          >
            Next: Optional Membership
            <ArrowRight size={16} />
          </button>
        </div>
      </div>
            ...
            >
              ...
              <div className="text-center space-y-3">
                ...
                <h3 className="text-xl font-black text-slate-900 dark:text-white">
                  SAATHAPP Wholesale Payment Gateway
                </h3>
                <p className="text-xs text-slate-500 font-medium">
                  Completing One-Time Onboarding Fee Payment of <strong className="text-emerald-600 dark:text-emerald-400">₹{feeCalc.fee.toLocaleString('en-IN')}</strong>
                </p>
              </div>
              ...
              <form onSubmit={handleConfirmPayment} className="mt-6 space-y-4 text-xs">
                ...
                <button
                  ...
                  className="transition-all duration-200 active:scale-[0.98] disabled:opacity-60 disabled:cursor-not-allowed focus-visible:ring-2 focus-visible:ring-emerald-500/50 focus-visible:outline-none w-full mt-4 flex items-center justify-center gap-2 rounded-2xl bg-emerald-600 hover:bg-emerald-500 py-3.5 text-xs font-black text-white shadow-xl transition hover:scale-[1.02] disabled:opacity-50"
                >
                  <Lock size={14} />
