...
import { workerPerformance } from '../../data/mockData';

const badgeIcons = { Award, Clock, Star, Zap, Trophy };

export default function PerformanceCard() {
  const metrics = [
    { label: 'Completion Rate', value: `${workerPerformance.completionRate}%`, desc: 'Jobs completed successfully', icon: ShieldCheck, color: 'text-primary' },
    { label: 'Acceptance Rate', value: `${workerPerformance.acceptanceRate}%`, desc: 'Jobs accepted from dispatches', icon: Flame, color: 'text-rose-500' },
    { label: 'Avg Job Time', value: workerPerformance.averageJobTime, desc: 'Average service turnaround', icon: Clock, color: 'text-blue-500' },
    { label: 'Customer Rating', value: `${workerPerformance.customerRating} ★`, desc: 'Weighted feedback score', icon: Star, color: 'text-amber-500' },
  ];

  const leaderboard = [
    { rank: 1, name: 'Vijay Singh', category: 'Electrician', rating: 4.95, completed: 184 },
    { rank: workerPerformance.rank, name: 'Ramesh Kumar (You)', category: 'Electrician', rating: workerPerformance.customerRating, completed: 142, highlight: true },
  ...
  return (
    <div className="space-y-6 text-left">
        ...
        {metrics.map((m, idx) => {
          ...
          return (
            <motion.div
              ...
              whileHover={{ y: -3 }}
              className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border border-slate-200/60 dark:border-slate-800 p-5 rounded-card shadow-soft hover:shadow-premium transition-all"
            >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border border-slate-200/60 dark:border-slate-800 p-6 rounded-card shadow-soft">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-sm font-black text-slate-800 dark:text-white uppercase tracking-wider">Leaderboard</h3>
            <span className="text-[10px] font-bold text-slate-400">Rank {workerPerformance.rank} of {workerPerformance.totalWorkers}</span>
          </div>
        ...
        <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border border-slate-200/60 dark:border-slate-800 p-6 rounded-card shadow-soft">
          ...
          <div className="space-y-4">
            {workerPerformance.badges.map((b) => {
