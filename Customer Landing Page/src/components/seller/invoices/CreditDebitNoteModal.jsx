    ...
    <SellerOverlay open={open} onClose={onClose} labelledBy="cdn-title">
      <div className="w-full max-w-md rounded-2xl bg-surface border border-slate-200 dark:border-slate-800 shadow-2xl p-5 space-y-4">
          ...
          {['credit', 'debit'].map((t) => (
            <button
              ...
              className={`flex-1 py-2 rounded-xl text-sm font-semibold border capitalize ${
                type === t ? 'border-emerald-500 bg-emerald-50 text-emerald-700' : 'border-slate-200'
              }`}
            >
              {t} note
            </button>
        ...
        <label className="block text-xs text-slate-500">
          Amount (₹)
          <input type="number" value={amount} onChange={(e) => setAmount(e.target.value)} className="mt-1 w-full rounded-xl border border-slate-200 bg-transparent px-3 py-2 text-sm" />
        </label>
        <label className="block text-xs text-slate-500">
