  ...
  if (canTransition(order.status, ORDER_STATUS.ACCEPTED)) {
    return (
      <div className="flex items-center gap-1">
        ...
        {btn('details', Eye, 'border border-slate-200 dark:border-slate-700 hover:bg-slate-50', 'View')}
      </div>
    );
  }
  if ([ORDER_STATUS.ACCEPTED, ORDER_STATUS.PREPARING, ORDER_STATUS.PACKED].includes(order.status)) {
    return (
      <div className="flex items-center gap-1">
        ...
        {btn('print_invoice', Printer, 'border border-slate-200 dark:border-slate-700 hover:bg-slate-50', 'Print')}
        {btn('details', Eye, 'border border-slate-200 dark:border-slate-700 hover:bg-slate-50', 'View')}
      </div>
    );
  }
  if (order.status === ORDER_STATUS.READY || order.status === ORDER_STATUS.PICKUP_ASSIGNED) {
    return (
      <div className="flex items-center gap-1">
        ...
        {btn('print_label', Printer, 'border border-slate-200 dark:border-slate-700 hover:bg-slate-50', 'Label')}
        {btn('details', Eye, 'border border-slate-200 dark:border-slate-700 hover:bg-slate-50', 'View')}
      </div>
    );
  }
  if ([ORDER_STATUS.OUT_FOR_DELIVERY, ORDER_STATUS.PICKED_UP].includes(order.status)) {
    return (
      <div className="flex items-center gap-1">
        ...
        {btn('call', Phone, 'border border-slate-200 dark:border-slate-700 hover:bg-slate-50', 'Call')}
        {btn('directions', MapPin, 'border border-slate-200 dark:border-slate-700 hover:bg-slate-50', 'Map')}
      </div>
    );
  }
  return (
...
export default function OrdersTable({
  ...
  if (!orders.length) {
    return (
      <div className="rounded-xl border border-dashed border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-12 text-center">
      ...
      <div className="hidden lg:block rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 overflow-hidden">
        <div className="overflow-x-auto max-h-[62vh]">
          <table className="w-full text-sm min-w-[1100px]">
            <thead className="sticky top-0 z-10 bg-slate-50 dark:bg-slate-800/95 border-b border-slate-200 dark:border-slate-800">
            ...
            <tbody>
              {orders.map((order) => (
                <tr
                  ...
                  className="border-b border-slate-100 dark:border-slate-800 hover:bg-slate-50/90 dark:hover:bg-slate-800/40 cursor-pointer transition-colors"
                >
      ...
      <div className="lg:hidden space-y-2.5">
        {orders.map((order) => (
          <article
            key={order.id}
            onClick={() => onRowClick?.(order)}
            className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-3.5 space-y-2.5"
          >
        ...
        <div className="flex items-center gap-3 flex-wrap">
          <label className="flex items-center gap-2 text-xs text-slate-500">
            ...
            <select
              ...
              className="rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 px-2 py-1"
            >
              {[6, 8, 10, 20].map((n) => (
          ...
          <div className="flex items-center gap-1">
            <button
              ...
              className="h-8 w-8 inline-flex items-center justify-center rounded-lg border border-slate-200 dark:border-slate-700 disabled:opacity-40"
            >
              <ChevronLeft size={14} />
            </button>
