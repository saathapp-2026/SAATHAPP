        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
          {tab === 'preview' ? (
            <div className="space-y-3">
              ...
              <div className="rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-950 min-h-[280px] flex items-center justify-center overflow-auto p-4">
          ...
          {tab === 'meta' ? (
            <dl className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
              {[
                ...
                <div key={k} className="rounded-xl bg-slate-50 dark:bg-slate-950 px-3 py-2">
                  <dt className="text-xs text-slate-400">{k}</dt>
                  <dd className="font-medium text-slate-800 dark:text-slate-100 mt-0.5 break-all">{v}</dd>
                </div>
              ))}
              <div className="sm:col-span-2 rounded-xl bg-slate-50 dark:bg-slate-950 px-3 py-2">
...
function IconBtn({ onClick, icon: Icon, label }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className="inline-flex items-center gap-1 rounded-lg border border-slate-200 dark:border-slate-700 px-2.5 py-1.5 text-xs font-semibold text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-800"
    >
      <Icon size={14} /> {label}
    </button>
  );
