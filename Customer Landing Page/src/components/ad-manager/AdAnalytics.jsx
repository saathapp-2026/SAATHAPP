...
export default function AdAnalytics({ data, loading, range, onRangeChange }) {
  ...
  return (
    <section className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-5 shadow-sm space-y-4" aria-label="Ad analytics">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <h2 className="text-base font-bold">Advertisement Analytics</h2>
        <div className="flex flex-wrap gap-1">
          {RANGES.map((r) => (
            <button key={r.id} type="button" onClick={() => onRangeChange?.(r.id)} className={`rounded-full px-2.5 py-1 text-[10px] font-semibold border ${range === r.id ? 'bg-emerald-600 text-white border-emerald-600' : 'border-slate-200 dark:border-slate-700'}`}>{r.label}</button>
          ))}
        </div>
      </div>
