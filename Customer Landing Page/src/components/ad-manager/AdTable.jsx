...
function ActionsMenu({ ad, onAction, loadingId }) {
  ...
  return (
    <div className="relative flex items-center justify-end gap-0.5">
      <button type="button" title="View" disabled={busy} onClick={() => onAction('view', ad)} className="p-1.5 rounded-lg hover:bg-page text-slate-500">
        <Eye size={15} />
      </button>
      <button type="button" title={ad.status === 'paused' ? 'Resume' : 'Pause'} disabled={busy} onClick={() => onAction(ad.status === 'paused' ? 'resume' : 'pause', ad)} className="p-1.5 rounded-lg hover:bg-page text-slate-500">
        {ad.status === 'paused' ? <Play size={15} /> : <Pause size={15} />}
      </button>
      <button type="button" title="Delete" disabled={busy} onClick={() => onAction('delete', ad)} className="p-1.5 rounded-lg hover:bg-page text-slate-500">
        <Trash2 size={15} />
      </button>
      <button type="button" title="More" disabled={busy} onClick={() => setOpen((v) => !v)} className="p-1.5 rounded-lg hover:bg-page text-slate-500" aria-haspopup="menu" aria-expanded={open}>
      ...
      {open ? (
        <>
          ...
          <div role="menu" className="absolute right-0 top-8 z-20 w-48 rounded-xl border border-slate-200 bg-surface shadow-xl py-1 max-h-72 overflow-y-auto">
            {items.map((a) => {
              const Icon = a.icon;
              return (
                <button key={a.id} type="button" role="menuitem" onClick={() => { setOpen(false); onAction(a.id, ad) }} className={`flex w-full items-center gap-2 px-3 py-2 text-left text-xs font-medium hover:bg-page ${a.danger ? 'text-red-600' : 'text-slate-700 dark:text-slate-200'}`}>
                  <Icon size={13} />
                  {a.label}
                </button>
              );
    ...
    return (
      <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-surface p-6 animate-pulse space-y-3" aria-busy="true">
        {Array.from({ length: 5 }).map((_, i) => (
          <div key={i} className="h-14 rounded bg-slate-200" />
        ))}
      </div>
    );
  }
  ...
  return (
    <section aria-label="Your advertisements">
      ...
      <div className="md:hidden space-y-2">
        {ads.map((ad) => (
          <div key={ad.id} className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-surface p-3 shadow-sm">
            <div className="flex items-start gap-2">
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="flex flex-wrap gap-1.5 mt-1.5">
                  <span className="rounded-full bg-page px-2 py-0.5 text-[10px] font-semibold">{getAdType(ad.typeId).short}</span>
          ...
          <table className="w-full text-left text-sm">
            <thead className="sticky top-0 z-[1] bg-slate-50/95 dark:bg-slate-950/95 backdrop-blur border-b border-slate-200 dark:border-slate-800">
              <tr className="transition-colors hover:bg-emerald-50/30 text-[11px] uppercase tracking-wide text-slate-500">
                <th className="px-3 py-3 w-10"><input type="checkbox" checked={allSelected} onChange={onToggleAll} aria-label="Select all" className="transition-colors duration-200 focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 outline-none rounded border-slate-300" /></th>
            ...
            <tbody>
              {ads.map((ad) => (
                <tr key={ad.id} className="transition-colors hover:bg-emerald-50/30 border-b border-slate-100 dark:border-slate-800 hover:bg-slate-50/80">
                  ...
                  <td className="px-3 py-3">
                    <div className="flex items-start gap-2.5 min-w-0">
                      <div className="h-9 w-9 rounded-lg bg-page inline-flex items-center justify-center shrink-0"><ImageIcon size={14} className="text-slate-400" /></div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 px-4 py-3 border-t border-slate-200 dark:border-slate-800 bg-slate-50/50 ">
          ...
          <div className="flex items-center gap-3">
            <label className="inline-flex items-center gap-2 text-xs text-slate-500">
              Rows per page
              <select value={pageSize} onChange={(e) => onPageSizeChange?.(Number(e.target.value))} className="rounded-lg border border-slate-200 bg-surface px-2 py-1 text-xs">
                {[5, 10, 20].map((n) => <option key={n} value={n}>{n}</option>)}
              </select>
            </label>
            <div className="flex gap-1">
              <button type="button" disabled={page <= 1} onClick={() => onPageChange(page - 1)} className="h-8 w-8 rounded-lg border border-slate-200 disabled:opacity-40 inline-flex items-center justify-center" aria-label="Prev"><ChevronLeft size={14} /></button>
              {Array.from({ length: Math.min(3, totalPages) }, (_, i) => i + 1).map((n) => (
                <button key={n} type="button" onClick={() => onPageChange(n)} className={`h-8 w-8 rounded-lg border text-xs font-semibold ${page === n ? 'bg-emerald-600 text-white border-emerald-600' : 'border-slate-200'}`}>{n}</button>
              ))}
              <button type="button" disabled={page >= totalPages} onClick={() => onPageChange(page + 1)} className="h-8 w-8 rounded-lg border border-slate-200 disabled:opacity-40 inline-flex items-center justify-center" aria-label="Next"><ChevronRight size={14} /></button>
            </div>
          </div>
        </div>
      </div>
