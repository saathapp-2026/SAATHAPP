      ...
      <main className="flex-1">
        ...
        <section className="py-12 bg-gradient-to-b from-white to-slate-50 dark:from-slate-900 dark:to-slate-950/40">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            {(() => {
              const isCustLoggedIn = isAuthenticated;
              
              return (
                <motion.div
                  onClick={() => navigate(isCustLoggedIn ? '/customer/dashboard' : '/customer')}
                  whileHover={{ y: -8, scale: 1.01 }}
                  transition={{ type: 'spring', stiffness: 300, damping: 20 }}
                ...
                >
                  ...
                  <div className="flex-1 space-y-6 text-left relative z-10">
                    <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/15 border border-white/20">
                      ...
                      <span className="text-[10px] font-black uppercase tracking-wider text-white">👤 {isCustLoggedIn ? 'Customer Portal Active' : 'Become a Customer'}</span>
                    </div>
                    
                    <div className="space-y-3">
                      <h3 className="text-2xl sm:text-4xl font-black text-white leading-tight drop-shadow-md">
                        {isCustLoggedIn ? `Welcome Back, ${user?.name || 'Customer'}` : 'Become a Customer'}
                      </h3>
                      <p className="text-sm sm:text-base text-white/90 font-medium leading-relaxed max-w-2xl drop-shadow">
                        {isCustLoggedIn 
                          ? 'Manage your grocery orders, technician bookings, and payments from your dashboard.'
                          : "Join India's trusted hyperlocal super app. Shop fresh groceries and hire certified service professionals."}
                      </p>
                    </div>
