...
export default function Topbar({
  ...
  return (
    <header className="sticky top-0 z-45 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border-b border-slate-200/60 dark:border-slate-800 py-3 px-4 sm:px-6 lg:px-8 flex items-center justify-between gap-4 shadow-sm">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="text-left hidden sm:block">
          <h1 className="text-sm font-black text-slate-800 dark:text-slate-100 uppercase tracking-wider flex items-center gap-1.5">
            <span>{greeting}, Ramesh Kumar</span>
            <Sparkles size={14} className="text-secondary" />
          </h1>
          <p className="text-[10px] text-slate-450 font-bold uppercase mt-0.5">{currentTime}</p>
        </div>
      </div>
      ...
      <div className="flex-1 max-w-xs md:max-w-md relative hidden md:block">
        ...
        <input
          type="text"
          placeholder="Search assigned jobs, tasks..."
      ...
      <div className="flex items-center gap-1.5 sm:gap-3">
        ...
        <div className="relative">
          <button
            ...
            className="flex items-center gap-1 px-2.5 py-1.5 rounded-xl border border-slate-200/60 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800 text-xs font-bold text-slate-655 dark:text-slate-300 cursor-pointer"
          >
            <Globe size={14} />
        ...
        <button
          ...
          onClick={() => alert('Check notifications log in Sidebar.')}
        >
          <Bell size={18} />
            ...
            {profileDropdown && (
              ...
              >
                <div className="px-3 py-2 border-b border-slate-100 dark:border-slate-800/60">
                  <p className="text-xs font-black text-slate-800 dark:text-slate-200">Ramesh Kumar</p>
                  <p className="text-[10px] text-slate-450 truncate">ramesh.kumar@saathapp.com</p>
                </div>
                <button
                  onClick={() => alert('Open profile tab.')}
                  className="w-full px-3 py-2 text-xs font-semibold text-slate-655 dark:text-slate-355 hover:bg-slate-50 dark:hover:bg-slate-800 flex items-center gap-2 cursor-pointer"
                >
