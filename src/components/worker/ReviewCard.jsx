...
export default function ReviewCard() {
  const [reviews, setReviews] = useState([
    { id: 1, author: 'Preeti Sharma', rating: 5, date: '2 days ago', text: 'Ramesh was extremely quick and checked all switchboards systematically. Very polite attitude.', reply: null },
    { id: 2, author: 'Aman Varma', rating: 5, date: 'Yesterday', text: 'Repaired our ATS box circuit efficiently. Solved the problem other electricians spent hours on.', reply: 'Thank you Aman! Glad I could fix the switch issue for you.' },
    { id: 3, author: 'Sunita Roy', rating: 4, date: '3 days ago', text: 'Checked light sockets and did clean work. Recommended electrician.', reply: null }
  ]);
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800 rounded-card p-6 shadow-soft hover:shadow-premium transition-all text-left flex flex-col justify-between">
        <div>
          ...
          <h3 className="text-base sm:text-lg font-black text-slate-800 dark:text-white mt-0.5">Performance Rating</h3>
        </div>

        <div className="flex items-center gap-4 my-6">
          <div className="text-center">
            <span className="text-5xl font-black text-slate-855 dark:text-white leading-none">4.90</span>
          ...
          <div className="space-y-1">
            ...
            <p className="text-xs text-slate-400 font-semibold">Based on 142 reviews</p>
          </div>
        </div>

        <div className="space-y-2">
          {starsDistribution.filter(d => d.stars > 0).map((dist, idx) => (
        ...
        <div className="space-y-5 overflow-y-auto max-h-[300px] pr-2 scrollbar-none">
          {reviews.map((rev) => (
            <div key={rev.id} className="pb-4 border-b border-slate-100 dark:border-slate-800/40 last:border-0 last:pb-0">
              ...
              {/* Review Text */}
              <p className="text-xs sm:text-sm text-slate-505 dark:text-slate-400 leading-relaxed mt-2">
                "{rev.text}"
              </p>
              ...
              {rev.reply && (
                <div className="mt-3 pl-4 border-l-2 border-primary/45 flex gap-2 items-start bg-slate-50/50 dark:bg-slate-950/20 p-2.5 rounded-r-xl">
                  ...
                  <div className="text-xs leading-relaxed text-slate-655 dark:text-slate-400">
